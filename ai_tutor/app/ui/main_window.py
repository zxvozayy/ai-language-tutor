...
from app.ui.progression_widget import ProgressionWidget, XPGainPopup
...
QDialog QLineEdit, QInputDialog QLineEdit {
    ...
    padding: 8px 10px;
        ...
        self.progression_widget = ProgressionWidget()
        self.progression_widget.setMaximumWidth(320)
        tabs.setCornerWidget(self.progression_widget, QtCore.Qt.TopRightCorner)
        ...
        QtCore.QTimer.singleShot(1500, self._check_daily_login_bonus)
    ...
    def _append_user_with_grammar(self, text: str):
        """Append user message with grammar highlighting and award XP."""
        ...
        if not callable(checker):
            ...
            # Award XP even without grammar check
            self._award_chat_xp(has_errors=False, error_count=0)
            return
        ...
        self._award_chat_xp(has_errors=bool(errors), error_count=len(errors))
    ...
    def _check_daily_login_bonus(self):
        """Check and award daily login XP bonus."""
        try:
            service = get_progression_service()
            event = service.check_daily_login()
            if event:
                self._show_xp_popup(event.xp_amount, "Daily login bonus! 🎉")
        except Exception as e:
            print(f"[MainWindow] Daily login check error: {e}")

    def _show_xp_popup(self, xp: int, description: str = ""):
        """Show XP gain popup animation."""
        try:
            popup = XPGainPopup(xp, description, self)
            # Position near the progression widget
            if hasattr(self, 'progression_widget'):
                pos = self.progression_widget.mapToGlobal(
                    QtCore.QPoint(0, self.progression_widget.height() + 10)
                )
            else:
                pos = self.mapToGlobal(QtCore.QPoint(100, 100))
            popup.show_at(pos)

            # Refresh progression widget
            if hasattr(self, 'progression_widget'):
                self.progression_widget.refresh()
        except Exception as e:
            print(f"[MainWindow] XP popup error: {e}")

    def _award_chat_xp(self, has_errors: bool, error_count: int = 0):
        """Award XP for sending a chat message."""
        try:
            service = get_progression_service()
            event = service.award_xp("chat_message", {
                "has_errors": has_errors,
                "error_count": error_count,
            })
            if event:
                self._show_xp_popup(event.xp_amount, "Message sent!")
        except Exception as e:
            print(f"[MainWindow] Award chat XP error: {e}")

    def _award_vocab_xp(self, word: str):
        """Award XP for learning a new word."""
        try:
            service = get_progression_service()
            event = service.award_xp("vocab_learned", {"word": word})
            if event:
                self._show_xp_popup(event.xp_amount, f"Learned: {word}")
        except Exception as e:
            print(f"[MainWindow] Award vocab XP error: {e}")
        ...
        if self.user_id:
            ...
            if res == QtWidgets.QMessageBox.Yes:
                try:
                    ...
                    # ✅ Award XP for learning word
                    self._award_vocab_xp(word)
